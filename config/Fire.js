...
class Fire {
    ...
    ****************************** */
    addCourse = async (course) => {
        ...
        return new Promise(async (res, rej) => {
            try {
                this.checkIfCourseNameAlreadyTaken(course.nom).then(
                    async (canUse) => {
                        if (canUse == false) {
                            res(false);
                        } else {
                            var newCourseRef = this.firestore
                                .collection("cours")
                                .doc();
                            ...
                            let dbUser = this.firestore
                                .collection("users")
                                .doc(this.uid);
                            await dbUser.update({
                                coursEnseignant: firebase.firestore.FieldValue.arrayUnion(
                                    course
                                ),
                            });
                            res(true);
    ...
    getAllStudentsCourses = async () => {
        return new Promise(async (resolve, reject) => {
            ...
            await db.get().then((querySnapshot) => {
                let docs = querySnapshot.docs;
                for (let doc of docs) {
                    if (doc.etudiant == true) {
                        const course = doc.data().cours;
                        res.push(course);
                    }
                }
            });
    ...
    ****************************** */
    ...
    postPdfToCourse = async (courseUID, pdf) => {
        return new Promise(async (res) => {
            remoteUri = await this.uplaodFileAsync(
                pdf.uri,
                `courses/${courseUID}/${pdf.name}`
            );
